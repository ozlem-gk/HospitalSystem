...
{
    public partial class CikisBilgisiMstr : System.Web.UI.Page
    {
        ...
        private void cikis()
        {
            ...
            MySqlCommand comm = new MySqlCommand("SELECT `id`, `cikis_tarihi`, `hasta_bilgi_id` FROM `cikis_bilgisi` WHERE 1", conn);
            try
            {
                //Bağlantımı açıyorum, dataReader nesnemi çalıştırıyorum ve GridView kontrolüme DataBind işlemi yapıyorum.
                conn.Open();
                reader = comm.ExecuteReader();
                grd_çikis.DataSource = reader;
                grd_çikis.DataBind();
